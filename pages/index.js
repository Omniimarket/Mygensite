...
const HomePage = ({ featuredGenerators }) => { // Receive featuredGenerators as props
  return (
    <div className="min-h-screen flex flex-col bg-gray-900 text-gray-100">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        <section className="text-center py-16 bg-gradient-to-r from-blue-600 to-purple-700 text-white rounded-2xl shadow-xl mb-12 transform hover:scale-[1.01] transition-transform duration-300 ease-in-out">
          ...
          <Link href="/all-generators" className="bg-white text-blue-700 hover:bg-blue-100 px-10 py-4 rounded-full text-lg font-semibold shadow-xl transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-2xl">Explore Generators</Link>
        </section>
        ...
        <section id="featured-generators" className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {featuredGenerators.map((gen, index) => (
              <Link key={index} href={gen.href} className={`generator-card bg-gray-800 p-8 rounded-xl shadow-lg flex flex-col items-center text-center border border-blue-700 hover:shadow-xl hover:border-${gen.color}-500 transform hover:scale-[1.02] transition-all duration-300 ease-in-out`}>
...
export async function getStaticProps() {
  // featuredGenerators is directly available here
  return {
    props: {
      featuredGenerators,
    },
    // revalidate: 3600, // Optional: re-generate page every hour if featuredGenerators change
  };
}
