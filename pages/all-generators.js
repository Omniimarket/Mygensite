...
const allGeneratorsData = {
  ...
  "Creative & Media Titles": [
    { name: "Book Title", href: "/book-title-generator", description: "Generate captivating titles for your books." },
    { name: "Song Title", href: "/song-title-generator", description: "Discover catchy and memorable titles to inspire your next musical hit." },
...
const AllGeneratorsPage = ({ categorizedGenerators }) => {
  return (
    <div className="min-h-screen flex flex-col bg-gray-900 text-gray-100">
      <Head>
        ...
        <meta name="keywords" content="all generators, AI tools, name generators, content generators, marketing tools, free AI generators" />
        {/* Canonical tag for All Generators page */}
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        <section className="bg-gray-800 p-8 md:p-12 rounded-2xl shadow-xl mb-12 border border-blue-700">
          ...
          {Object.entries(categorizedGenerators).map(([category, generators]) => (
            <div key={category} className="mb-10">
              <h2 className="text-3xl font-bold text-blue-300 mb-6 text-center md:text-left border-b-2 border-blue-600 pb-3">
                {category}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {generators.map((generator) => (
                  <Link key={generator.href} href={generator.href} className="block bg-gray-700 p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-200 ease-in-out transform hover:scale-[1.02] border border-blue-600 hover:border-blue-500">
                    <h3 className="text-xl font-semibold text-blue-300 mb-2">{generator.name} Generator</h3>
...
export async function getStaticProps() {
  return {
    props: {
      categorizedGenerators: allGeneratorsData, // Pass the categorized data as props
    },
    // Optional: revalidate every X seconds if your generator data could change
